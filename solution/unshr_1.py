...
def computeDistance(i,j):
    ...
    c1, c2, d = (i + 1) * shredWidth - 1, j * shredWidth, 0
    # the distance is the cumulated difference of the pixels along this sewing
    for k in range(imageHeight): d += np.linalg.norm(pixels[k][c1]-pixels[k][c2])
    return d
...
pixels = np.asarray(img.getdata()).reshape(img.height,img.width,len(img.getbands()))
